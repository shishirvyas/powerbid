...
Last updated: 2026-05-02 (Workflow Control BOM to PO handoff)
...
## Change Log
- 2026-05-02: Enhanced Workflow Control handoff to supplier-aware procurement generation. BOM item rows now support optional Supplier Product mapping, and Workflow Control now shows Supplier Ready count per BOM card. Generate PO now creates draft Purchase Orders grouped by supplier (one per supplier group) from mapped BOM lines, skips unmapped lines, and prevents duplicates when an open PO already exists for the BOM.
- 2026-05-02: Added Workflow Control under Procurement for BOM-to-Procurement handoff. New card-based queue shows BOMs with SO context, material count, active status, and open PO count. Added one-click Generate PO action that creates a draft Purchase Order directly from BOM raw-material lines and links it back to the BOM/SO for traceability. Generation is idempotent for open POs per BOM (draft/pending/approved/sent): if an open PO already exists, system routes user to existing PO instead of creating a duplicate.
- 2026-05-02: Changed BOM Items field from Wastage % to Unit Price for per-unit cost basis. Field type: decimal number (min 0, step 0.01). Enables accurate per-line-item cost calculation (qty/unit × unitPrice) for material cost rollup. When BOM items are used in production orders, base quantity is calculated without wastage factor adjustments: qtyPlanned = qtyPerUnit × plannedQty. Updated database schema (migration 0014_bom_items_unit_price.sql), validation schema, and calculation functions to support price-based cost flow instead of percentage-based adjustments.
- 2026-05-02: Enhanced SO lock/unlock safeguard: When a Sales Order is linked to a BOM, the Sales Order field itself is locked by default to prevent accidentally unlinking it. All other form fields (Product, BOM Code, Version, Items, etc.) remain editable. Users must click the Unlock button to change or remove the linked SO. Lock state automatically releases when the SO is unlinked or the dialog closes.
- 2026-05-02: Added SO lock/unlock UI safeguard to BOM create/edit dialog. When a Sales Order is linked, a Lock/Unlock button appears next to the SO field. When locked, all form inputs (Product, BOM Code, Version, Active flag, Labor/Overhead costs, Notes, and BOM Items) are disabled to prevent accidental edits. Users must click Unlock to make changes. Lock state resets when the dialog closes. Submit button is also disabled when SO is locked. Added column headers (Raw Material, Qty/Unit, Wastage %, Notes) above BOM Items grid on desktop (md+) for improved field clarity.
- 2026-05-02: Updated BOM create/edit dialog behavior for SO-linked flow. Sales Order selector and linked SO details panel are now at the top. When an SO is linked, Product and BOM Code fields are hidden from UI (auto-inferred/retained in form state). If no SO is linked, Product and BOM Code fields are shown and editable as before. BOM Items entry is now LAN Product only (supplier-product toggle removed from BOM dialog UI).
